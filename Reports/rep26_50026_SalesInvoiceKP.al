report 50026 "Sales - Invoice - KP"
{
    ...
    // TAL0.4 2021/11/03 VC Design COC No.
    // TAL0.5 2021/11/03 VC Option to Hide Global Gap

    //ship-to address hide =(Fields!ShipToAddr1.Value = "") 
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/rep26_50026_SalesInvoiceKP.rdlc';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(VATRegNo_SalesInvHdr; "Sales Invoice Header"."VAT Registration No." + ' ' + vG_TIC)
                    {
                    }
                    ...
                    column(CustRefNo; "Sales Invoice Header"."Customer Reference No.")
                    {
                    ...
                    column(Cust_ShowTotalQty; rG_Customer."Show Total Qty")
                    {
                    }

                    column(TotalQty; vG_TotalQty)
                    {
                        DecimalPlaces = 0 : 5;
                    }

                    column(MandatoryCYFields; rG_Customer."Mandatory CY Fields")
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        //+1.0.0.297
                        column(CountryRegionofOriginCode; "Sales Invoice Line"."Req. Country")
                        {
                        }
                        ...
                        column(Category9; "Sales Invoice Line"."Category 9")
                        {
                        }
                        ...
                        begin
                            ...
                            if "Sales Invoice Line".Type = "Sales Invoice Line".Type::Item then
                                if zitem.Get("Sales Invoice Line"."No.") then
                                    zshelfno := zitem."Shelf No.";

                            //zshelfno shown only for Lidl
                            vG_ItemReferenceNo := '';
                            if ("Sales Invoice Header"."Sell-to Customer No." = 'CUST00032') and (zshelfno <> '') then begin
                                vG_ItemReferenceNo := zshelfno;
                            end;

                            if vG_ItemReferenceNo = '' then begin
                                vG_ItemReferenceNo := "Sales Invoice Line"."Item Reference No.";
                            end;
                            ...
                            if rG_Customer."Mandatory CY Fields" then begin
                                CountryofOriginCaption := 'Country of Origin';
                                ClassCaption := 'Class';
                                ProductClass := "Sales Invoice Line"."Product Class";

                                if Country.get("Sales Invoice Line"."Country/Region of Origin Code") then begin
                                    CountryofOriginName := Country.Name;
                                end;

                                if "Sales Invoice Line"."Country/Region of Origin Code" <> '' then begin
                                    if not Country.get("Sales Invoice Line"."Country/Region of Origin Code") then begin
                                        Message(Txt50010, "Sales Invoice Line"."Country/Region of Origin Code");
                                    end;
                                end;


                                if GenCat.GET(27, GenCat.Type::Category9, "Sales Invoice Line"."Category 9") then begin
                                    PotatoesDesc := GenCat.Description;
                                    "Sales Invoice Line".Description += ' (' + PotatoesDesc + ')';
                                end;
                            end;
            ...
            trigger OnAfterGetRecord();
            var
                vL_SalesInvoiceLine: Record "Sales Invoice Line";
            begin
                ...
                if "Sales Invoice Header"."Sell-to County" = "Sales Invoice Header"."Sell-to City" then begin
                    "Sales Invoice Header"."Sell-to County" := '';
                end;

                if "Sales Invoice Header"."Bill-to County" = "Sales Invoice Header"."Bill-to City" then begin
                ...
                if "Sales Invoice Header"."Ship-to County" = "Sales Invoice Header"."Ship-to City" then begin
                ...
                //if "Sales Invoice Header"."Customer Reference No." <> '' then begin
                //    "Sales Invoice Header"."Customer Reference No." := 'Reference No.: ' + "Sales Invoice Header"."Customer Reference No.";
                //end;
                ...
                if rG_Customer."Show Total Qty" then begin
                    vL_SalesInvoiceLine.RESET;
                    vL_SalesInvoiceLine.SetFilter("Document No.", "Sales Invoice Header"."No.");
                    vL_SalesInvoiceLine.SetRange(Type, vL_SalesInvoiceLine.Type::Item);
                    if vL_SalesInvoiceLine.FindSet() then begin
                        repeat
                            vG_TotalQty += vL_SalesInvoiceLine.Quantity;
                        until vL_SalesInvoiceLine.next = 0;

                        if rG_Customer."Report Decimal Places" then begin
                            EVALUATE(vG_TotalQty, FORMAT(vG_TotalQty, 0, '<Precision,0:1><Standard Format,0>'));

                        end else begin
                            EVALUATE(vG_TotalQty, FORMAT(vG_TotalQty, 0, '<Precision,0:0><Standard Format,0>'));
                        end;
                    end;
                end;
            end;
