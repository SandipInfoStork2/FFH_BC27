report 50063 "Check FFH"
{
    // TAL0.1 2019/07/15 VC review addresss 2-5 lines, always the same address and second page onwards
    // TAL0.2 2019/12/17 ANP Added payee name for cheque name
    // TAL0.3 2020/02/04 VC review more than 24 applied invoices
    // TAL0.4 2020/02/24 VC Cheques multiple
    //                       removed from footer section show visibility from rectangle
    //                             =iif(CBool(ReportItems!IsLastRec.Value),False,True)
    DefaultLayout = RDLC;
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(vG_JournalLinePrint; vG_JournalLinePrint)
            {
            }
            dataitem(CheckPages; "Integer")
            {
                ...
                column(zCheckToAddr2; CheckToAddr[2])
                {
                }
                column(zCheckToAddr4; CheckToAddr[4])
                {
                }
                column(zCheckToAddr3; CheckToAddr[3])
                {
                }
                column(zCheckToAddr5; CheckToAddr[5])
                {
                }
                column(Vendor_No; Vend."No.")
                {
                }
                column(vG_PrintApplicationSummary; vG_PrintApplicationSummary)
                {
                }
                column(vG_ApplicationSummaryText; vG_ApplicationSummaryText)
                {
                }
                dataitem(PrintSettledLoop; "Integer")
                {
                    ...
                    begin
                        ...
                        //+TAL0.1
                        if vG_PrintApplicationSummary then
                            FoundLast := true;
                        //-TAL0.1
                    end;
                ...
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    column(TestAmt; TestAmt)
                    {
                    }
                    column(TestAmt1; TestAmt1)
                    {
                    }
                    column(TestAmt2; TestAmt2)
                    {
                    }
                ...
                begin
                    ...
                    PageSplit := GLSetup."Cheque Page Line No";
                    if PageSplit = 0 then
                        Error('PAGESPLIT NOT SET');
                    ...
                    case BalancingType of
                        BalancingType::Customer:
                            begin
                                ...
                                //MESSAGE('CustLedgEntry:'+FORMAT(CustLedgEntry.COUNT));
                            end;
                        ...
                        BalancingType::Vendor:
                            begin
                                ...
                                VendLedgEntry.SetRange(Open, true);
                                VendLedgEntry.SetRange("Applies-to ID", GenJnlLine."Applies-to ID");
                                //MESSAGE('VendLedgEntry:'+FORMAT(VendLedgEntry.COUNT));
                    ...
                    if RowCounter <= (PageSplit - 1) then begin
                        ...
                        //PageCounter:=1 + ((RowCounter-(PageSplit-1))/PageSplit )
                    ...
                    if RowCounter <= (PageSplit) then begin
                        vG_PrintApplicationSummary := false;
                        vG_ApplicationSummaryText := '';
                    end else begin
                        vG_PrintApplicationSummary := true;
                        vG_ApplicationSummaryText := StrSubstNo('Payment on Account. Total Invoices: %1. Please see Payment Receipt for details.', RowCounter);
                        FoundLast := true;
                    end;
                    ...
                    CheckToAddr[1] := vG_CheckToAddress1Temp; //TAL0.3
                end;
                    ...
                    case BalancingType of
                        BalancingType::"G/L Account":
                            begin
                                ...
                                vG_CheckToAddress1Temp := CheckToAddr[1]; //TAL0.3
                            end;
                        BalancingType::Customer:
                            begin
                        ...
                        BalancingType::Vendor:
                            begin
                                ...
                                //TAL0.2
                                GenJnlLine.TestField("Payee Name (GL Cheque)");
                                CheckToAddr[1] := GenJnlLine."Payee Name (GL Cheque)";
                                vG_CheckToAddress1Temp := CheckToAddr[1]; //TAL0.3
                                                                          //TAL0.2
                                if BankAcc2."Currency Code" <> "Currency Code" then
                                    Error(Text005);
    ...
    var
        ...
        TestAmt2: Text[30];
        TestAmt: Text[30];
        Text066: Label 'VOID VOID VOID VOID VOID VOID';
        vG_CheckToAddress1Temp: Text;
        vG_PrintApplicationSummary: Boolean;
        vG_ApplicationSummaryText: Text;
        vG_JournalLinePrint: Integer;
        RowCounter: Integer;
        PageCounter: Integer;
        PageSplit: Integer;
