report 50092 "Item Register - Value FFH"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/rep92_50092_ItemRegisterValue.rdlc';
    ...
    dataset
    {
        dataitem("Item Register"; "Item Register")
        {
            ...
            dataitem("Value Entry"; "Value Entry")
            {
                ...
                {
                    IncludeCaption = true;
                    // AutoFormatType = 2; //TAL
                    // DecimalPlaces = 0 : 2;//TAL
                    //"Cost per Unit"
                    //   EVALUATE("Cost per Unit", FORMAT("Quantity (Base)", 0, '<Precision,0:2><Standard Format,0>'));
                    //=Replace(Parameters!CostperUnit_ValueEntryCaption.Value," ",chr(10))
                    //=Fields!CostperUnit_ValueEntryFormat.Value
                }
                ...
                begin
                    ...
                    if vG_StandardDecimals then begin
                        //EVALUATE("Cost per Unit", FORMAT("Cost per Unit", 0, '<Precision,0:2><Standard Format,0>'))
                    end else begin


                        "Cost per Unit" := Round("Cost per Unit", 0.01);
                        "Invoiced Quantity" := Round("Invoiced Quantity", 0.01);

                        //EVALUATE("Cost per Unit", FORMAT("Cost per Unit", 0, '<Precision,0:2><Standard Format,0>'));
                        //EVALUATE("Invoiced Quantity", FORMAT("Invoiced Quantity", 0, '<Precision,0:2><Standard Format,0>'))
        ...
        layout
        {
            area(Content)
            {
                group(Options)
                {
                    ...
                    field(vG_StandardDecimals; vG_StandardDecimals)
                    {
