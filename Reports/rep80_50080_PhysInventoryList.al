report 50080 "Phys. Inventory List FFH"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/rep80_50080_PhysInventoryList.rdlc';
    ApplicationArea = Warehouse;
    Caption = 'Physical Inventory List FFH';
        ...
        if ItemLedgEntry.FindSet then
            repeat
                CreateReservEntry(ItemJnlLine, ItemLedgEntry."Remaining Quantity",
                  ItemLedgEntry."Serial No.", ItemLedgEntry."Lot No.", ItemLedgEntry."Item Tracking", ItemLedgEntry."Expiration Date"); //TAL
            ...
            repeat
                if (WhseEntry."Lot No." <> '') and (WhseEntry."Serial No." <> '') then
                    CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)",
                      WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackingEntryType::"Lot and Serial No.", WhseEntry."Expiration Date") //TAL
                else begin
                    if WhseEntry."Lot No." <> '' then
                        CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)",
                          WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackingEntryType::"Lot No.", WhseEntry."Expiration Date"); //TAL
                    if WhseEntry."Serial No." <> '' then
                        CreateReservEntry(ItemJnlLine, WhseEntry."Qty. (Base)",
                          WhseEntry."Serial No.", WhseEntry."Lot No.", ItemTrackingEntryType::"Serial No.", WhseEntry."Expiration Date"); //TAL
                end;
            until WhseEntry.Next() = 0;
    end;

    local procedure CreateReservEntry(ItemJournalLine: Record "Item Journal Line"; Qty: Decimal; SerialNo: Code[50]; LotNo: Code[50]; ItemTracking: Enum "Item Tracking Entry Type"; pExpDate: Date)
    var
        FoundRec: Boolean;
    begin
        ...
        if not FoundRec then begin
            ...
            ReservEntryBuffer."Expiration Date" := pExpDate; //TAL
