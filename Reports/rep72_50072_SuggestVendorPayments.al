// TAL0.1 2017/12/07 VC add logic to calculate with posting date, option from user
// TAL0.2 2019/12/17 ANP Add payee name = vendor name
report 50072 "Suggest Vendor Payments FFH"
{
                ...
                group(Options)
                {
                    ...
                    group("Find Payments")
                    {
                        ...
                        field(DateType; DateType)
                        {
                            ApplicationArea = All;
                            Caption = 'DateType';
                            OptionCaption = 'Posting Date,Due Date';
    ...
    begin
        ...
        if Future then begin
            ...
            //+TAL0.1
            if DateType = DateType::"Due Date" then begin
                VendLedgEntry.SetRange("Due Date", LastDueDateToPayReq + 1, DMY2Date(31, 12, 9999));
            end else begin
                VendLedgEntry.SetRange("Posting Date", LastDueDateToPayReq + 1, DMY2Date(31, 12, 9999));
            end;
            //-TAL0.1
            ...
            if DateType = DateType::"Due Date" then begin
                ...
                VendLedgEntry.SetRange("Posting Date", 0D, LastDueDateToPayReq);
            end;
            //-TAL0.1
        end;
    ...
    begin
        ...
        GenJnlLine."Payee Name (GL Cheque)" := Vend2.Name; //TAL0.1
        ...
        if DateType = DateType::"Due Date" then begin
            ...
            PayableVendLedgEntry.Future := (VendLedgEntry."Posting Date" > LastDueDateToPayReq);
        end;
        //-TAL0.1
    ...
    begin
        ...
        Clear(SourceDict);
        SourceDict.Add(
            DimMgt.TypeToTableID1(GenJnlLine."Account Type".AsInteger()),
        ...
        if GenJnlLine."Bal. Account No." <> '' then begin
            Clear(SourceDict);
            SourceDict.Add(
                DimMgt.TypeToTableID1(GenJnlLine."Bal. Account Type".AsInteger()),
        ...
        if GenJnlLine."Job No." <> '' then begin
            Clear(SourceDict);
            SourceDict.Add(Database::Job, GenJnlLine."Job No.");
            SourceList.Add(SourceDict);
        end;

        // Salesperson
        if GenJnlLine."Salespers./Purch. Code" <> '' then begin
            Clear(SourceDict);
            SourceDict.Add(Database::"Salesperson/Purchaser", GenJnlLine."Salespers./Purch. Code");
            SourceList.Add(SourceDict);
        end;

        // Campaign
        if GenJnlLine."Campaign No." <> '' then begin
            Clear(SourceDict);
            SourceDict.Add(Database::Campaign, GenJnlLine."Campaign No.");
            SourceList.Add(SourceDict);
        end;

        GenJnlLine.CreateDim(SourceList);
        //BC25 According <<
