report 50091 "Sales - Return Receipt FFH"
{
    DefaultLayout = RDLC;
        ...
        dataitem("Return Receipt Header"; "Return Receipt Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(CustAddr1H; CustAddr[1])
                    {
                    }
                    column(CustAddr2H; CustAddr[2])
                    {
                    }
                    column(CustAddr3H; CustAddr[3])
                    {
                    }
                    column(CustAddr4H; CustAddr[4])
                    {
                    }
                    column(CustAddr5H; CustAddr[5])
                    {
                    }
                    column(CustAddr6H; CustAddr[6])
                    {
                    }
                    column(CustAddr7H; CustAddr[7])
                    {
                    }
                    column(CustAddr8H; CustAddr[8])
                    {
                    }

                    column(ExtDocNo; "Return Receipt Header"."External Document No.")
                    {
                    }

                    column(PostingDt_ReturnRcptHeader; Format("Return Receipt Header"."Posting Date"))
                    {
                    }
                    ...
                    dataitem("Return Receipt Line"; "Return Receipt Line")
                    {
                        ...
                        begin
                            ...
                            zshelfno := '';
                            if "Return Receipt Line".Type = "Return Receipt Line".Type::Item then
                                if zitem.Get("Return Receipt Line"."No.") then
                                    zshelfno := zitem."Shelf No.";

                            //zshelfno shown only for Lidl
                            vG_ItemReferenceNo := '';
                            if ("Return Receipt Header"."Sell-to Customer No." = 'CUST00032') and (zshelfno <> '') then begin
                                vG_ItemReferenceNo := zshelfno;
                            end;

                            if vG_ItemReferenceNo = '' then begin
                                vG_ItemReferenceNo := "Return Receipt Line"."Item Reference No.";
                            end;
                        end;
